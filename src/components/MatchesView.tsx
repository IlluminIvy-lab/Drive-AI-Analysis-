...
import { isMatchSelectable } from '../lib/cleanupActionGate';
...
export const MatchesView: React.FC<MatchesViewProps> = ({
  ...
  const handleToggle = (id: string) => {
    const targetMatch = allCandidateMatches.find((m) => m.id === id);
    if (targetMatch && !isMatchSelectable(targetMatch)) {
      return; // Probable-candidate and unreadable files cannot be selected
    }
  ...
  const selectableFilteredMatches = useMemo(() => {
    return filteredMatches.filter(isMatchSelectable);
  }, [filteredMatches]);

  const allSelected =
    selectableFilteredMatches.length > 0 &&
    selectableFilteredMatches.every((m) => selectedMatchIds.includes(m.id));
            ...
            <button
              ...
              onClick={() => {
                if (allSelected) {
                  ...
                  const ids = selectableFilteredMatches.map((m) => m.id);
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2.5 min-w-0">
                      <button
                        ...
                        disabled={!isMatchSelectable(m)}
                        className={`mt-0.5 ${
                          !isMatchSelectable(m)
                            ? 'opacity-30 cursor-not-allowed text-[#555555]'
                            : 'text-[#A0988E] hover:text-[#C75B12] cursor-pointer'
                        }`}
                        aria-label={
                          !isMatchSelectable(m)
                            ? 'Cannot select candidate requiring manual review or unreadable content'
                            : isSelected
                            ? 'Deselect file'
                            : 'Select file'
                        }
                        title={
                          !isMatchSelectable(m)
                            ? 'Probable candidate or unreadable file requires manual review and cannot be selected for trashing.'
                            : undefined
                        }
                      >
                        {isSelected ? (
                          ...
                          <Square className={`w-4 h-4 ${!isMatchSelectable(m) ? 'text-[#444444]' : 'text-[#666666]'}`} />
                        )}
                      </button>
