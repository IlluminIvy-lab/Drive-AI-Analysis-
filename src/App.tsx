...
export default function App() {
  ...
  const [pendingMatchesToTrash, setPendingMatchesToTrash] = useState<DuplicateMatch[]>([]);
      ...
      for (let i = 0; i < rawFiles.length; i++) {
        ...
        try {
          ...
          const authoritativeSize =
            file.size !== undefined && Number(file.size) > 0
              ? Number(file.size)
              : sizeBytes !== undefined
              ? sizeBytes
              : file.size;
   ...
   * Enforces:
   * 1. Plan approval: must be explicitly confirmed via confirmation modal.
   * 2. Empty selection policy: empty selection strictly means ZERO actions.
   *    Never falls back to all actionable matches.
   * 3. cleanupActionGate: every trash operation is gated. Unreadable, probable-candidate,
   *    uncertain, divergent, and protected items cannot be trashed.
   */
  const handleConfirmTrash = async () => {
    const accessToken = token || getAccessToken();
    if (!accessToken) return;

    // 1. Candidate selection: strictly from pendingMatchesToTrash or explicitly selectedMatchIds
    const candidateMatches =
      pendingMatchesToTrash.length > 0
        ? pendingMatchesToTrash
        : (selectedMatchIds.length > 0
            ? actionableMatches.filter((m) => selectedMatchIds.includes(m.id))
            : []);

    // 2. Empty selection means zero actions. Never fall back to all actionable matches!
    if (candidateMatches.length === 0) {
      setIsConfirmationOpen(false);
      setPendingMatchesToTrash([]);
      return;
    }

    // 3. Filter through cleanupActionGate with explicit plan approval = true
    const gateResult = filterEligibleMatchesForTrash(candidateMatches, true);
    const matchesToTrash = gateResult.approvedMatches;

    if (matchesToTrash.length === 0) {
      setIsConfirmationOpen(false);
      setPendingMatchesToTrash([]);
      return;
    }
    ...
    for (let i = 0; i < matchesToTrash.length; i++) {
      ...
      try {
        // Enforce cleanupActionGate validation before every single trash call
        assertCanTrash(match, true);
  ...
  const handleTrashSingleMatch = (match: DuplicateMatch) => {
    // Evaluate eligibility via cleanupActionGate
    const check = evaluateTrashEligibility(match);
    if (!check.allowed) {
      alert(`Cannot trash "${match.targetFile.name}": ${check.reason}`);
      return;
    }

    // Plan must be explicitly approved via confirmation modal
    setPendingMatchesToTrash([match]);
    setIsConfirmationOpen(true);
  };

  const handleBulkTrashMatches = (matchesToTrash: DuplicateMatch[]) => {
    if (!matchesToTrash || matchesToTrash.length === 0) return;

    // Filter through cleanupActionGate with plan approval = true
    const gateResult = filterEligibleMatchesForTrash(matchesToTrash, true);
    if (gateResult.approvedMatches.length === 0) {
      alert('None of the selected items can be safely trashed by policy.');
      return;
    }

    // Plan must be explicitly approved via confirmation modal
    setPendingMatchesToTrash(gateResult.approvedMatches);
    setIsConfirmationOpen(true);
  };
  ...
  const handleExecuteUndo = async (action: HistoryAction) => {
    ...
    try {
      if (action.type === 'trash' || action.type === 'bulk_trash') {
        // Safe restoration: calls restoreFileFromTrash to return trashed files back to folder
            ...
            {scanStage === 'smart_review' && (
              <SmartScanReviewView
                ...
                onToggleSelectMatch={(id) => {
                  const match = actionableMatches.find((m) => m.id === id) || uncertainMatches.find((m) => m.id === id);
                  if (match && !isMatchSelectable(match)) {
                    return;
                  }
                ...
                onSelectAll={() => setSelectedMatchIds(actionableMatches.filter(isMatchSelectable).map((m) => m.id))}
                ...
                onConfirmTrashApproved={() => {
                  ...
                  const toTrash = actionableMatches.filter((m) => selectedMatchIds.includes(m.id));
                  const gateResult = filterEligibleMatchesForTrash(toTrash, true);
                  if (gateResult.approvedMatches.length === 0) {
                    alert('None of the selected items can be safely trashed by policy.');
                    return;
                  }
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#262626]">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      ...
                      <span>
                        Execute Trash ({selectedMatchIds.length})
                      </span>
                    </button>
                  </div>
                </div>
