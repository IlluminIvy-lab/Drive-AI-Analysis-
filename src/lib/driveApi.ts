...
/**
 * Safely decodes text from a Response without throwing "Failed to decode data" errors
 * even on invalid UTF-8 byte streams, non-standard encodings, or partial transfers.
 */
async function safelyReadResponseText(res: Response): Promise<string> {
  try {
...
export async function listDriveFolders(accessToken: string): Promise<DriveFolderItem[]> {
  ...
  try {
    do {
      ...
      if (!res.ok) {
        const err = await safelyReadResponseText(res);
...
export async function findFolderByName(
  ...
  if (!response.ok) {
    const errorText = await safelyReadResponseText(response);
...
export async function readFileContent(
  ...
  try {
    if (file.mimeType === 'application/vnd.google-apps.document') {
      ...
      if (res.ok) {
        const raw = await safelyReadResponseText(res);
      ...
      // Try server-side media proxy first to avoid browser 302 redirect dropping auth headers
      let res: Response | null = null;
      try {
        const proxyUrl = `/api/drive/media?fileId=${encodeURIComponent(file.id)}`;
        ...
        // Fallback to direct drive fetch if proxy unreachable
      }

      if (!res) {
        const directUrl = `${DRIVE_API_BASE}/files/${file.id}?alt=media`;
        res = await driveFetch(directUrl, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
      }
      ...
      if (res.ok) {
        if (isPdf) {
          ...
          const buffer = await res.arrayBuffer();
          // Detect if returned content is actually an HTML redirect/error page
          const firstBytes = new Uint8Array(buffer.slice(0, Math.min(buffer.byteLength, 100)));
          const firstStr = String.fromCharCode(...firstBytes).toLowerCase();
          const isHtmlError = isHtml || firstStr.includes('<!doctype') || firstStr.includes('<html') || firstStr.includes('servicelogin');
...
export async function moveFileToTrash(
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await safelyReadResponseText(res);
...
export async function restoreFileFromTrash(
  ...
  if (!res.ok) {
    const errText = await safelyReadResponseText(res);
    throw new Error(`Failed to restore file ${fileId}: ${res.status} - ${errText}`);
  }
