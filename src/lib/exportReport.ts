...
/**
 * Safely formats an ISO date string without throwing on invalid or null dates.
 */
export function safeFormatIsoDate(val: any): string {
  if (!val) return 'N/A';
  try {
...
/**
 * Safely formats a locale date string without throwing on invalid or null dates.
 */
export function safeFormatLocaleDate(val: any): string {
  if (!val) return 'N/A';
  try {
    const d = new Date(val);
    if (isNaN(d.getTime())) return 'N/A';
...
/**
 * Escapes a field for RFC 4180 CSV compliance and protects spreadsheet formula injection.
 */
function escapeCsv(val: any): string {
  if (val === undefined || val === null) return '""';
  let str = String(val);
  // Protect spreadsheet formula-like values (=, +, -, @, \t, \r)
  if (/^[=+\-@\t\r]/.test(str)) {
    str = `'${str}`;
  }
...
export function getReportFilename(report: CleanupReport, format: ReportExportFormat): string {
  const dateStr = safeFormatIsoDate(report.timestamp).split('T')[0] || 'date';
...
export function exportReportToCsv(report: CleanupReport): void {
  ...
  validTrashed.forEach((item) => {
    ...
    lines.push([
      ...
      escapeCsv(safeFormatIsoDate(file.modifiedTime)),
      escapeCsv(item.keptOriginalFile?.name || 'N/A'),
      escapeCsv(item.keptOriginalFile?.id || 'N/A'),
      escapeCsv(item.keptOriginalFile ? safeFormatIsoDate(item.keptOriginalFile.modifiedTime) : 'N/A'),
  ...
  report.uncertainFiles.forEach((item) => {
    ...
    lines.push([
      ...
      escapeCsv(safeFormatIsoDate(targetFile.modifiedTime)),
...
export function generateMarkdownReport(report: CleanupReport): string {
  ...
  if (validTrashed.length === 0) {
    ...
    validTrashed.forEach((item, index) => {
      ...
      const typeLabel = item.type === 'content-exact'
        ...
        ? 'Byte-Exact Duplicate (Binary Checksum)'
        : isExact
        ? 'Exact Duplicate'
        : 'Older Draft Version';
      ...
      const simLabel =
        item.comparisonMethod === 'size_and_name_match'
          ...
          : item.comparisonMethod === 'none'
          ? 'Unreadable Content'
          : item.signalUsed === 'canonical_export'
          ? 'Canonical Export Text'
          : item.signalUsed === 'content_hash'
      ...
      md += `- **Modified Date:** ${safeFormatLocaleDate(file.modifiedTime)}\n`;
      if (item.keptOriginalFile) {
        md += `- **Superseded By (Kept Copy):** "${item.keptOriginalFile.name}" (ID: \`${item.keptOriginalFile.id}\`, Modified: ${safeFormatLocaleDate(item.keptOriginalFile.modifiedTime)})\n`;
      }
  ...
  if (displayedKept.length === 0) {
    ...
    displayedKept.forEach((f) => {
      md += `| ${f.name} | ${safeFormatLocaleDate(f.modifiedTime)} | ${formatBytes(getEstimatedFileSize(f))} | \`${f.id}\` |\n`;
    });
    md += `\n`;
  }
...
export function generatePlainTextReport(report: CleanupReport): string {
  ...
  if (validTrashed.length === 0) {
    ...
    validTrashed.forEach((item, index) => {
      ...
      txt += `    - Modified Date:   ${safeFormatLocaleDate(file.modifiedTime)}\n`;
      if (item.keptOriginalFile) {
        txt += `    - Superseded By:   "${item.keptOriginalFile.name}" (ID: ${item.keptOriginalFile.id}, Modified: ${safeFormatLocaleDate(item.keptOriginalFile.modifiedTime)})\n`;
      }
...
export const exportReportToPlainText = exportReportToText;
