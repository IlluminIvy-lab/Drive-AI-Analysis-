import { DriveFileItem, DuplicateMatch, DuplicateType, AutoSelectPreferences } from '../types';
 ...
 * date stamps, test/category prefixes, and common copy suffixes (e.g. " (1)", "_v2", "-draft").
 */
export function getBaseTitle(filename: string): string {
  // Strip extension
  let base = filename.replace(/\.[^/.]+$/, '');
  // Strip test/category prefixes like "cat1_", "cat7_unique_", "test_"
  base = base.replace(/^cat\d+[\s_-]*(?:unique[\s_-]*)?/i, '');
  base = base.replace(/^test[\s_-]*/i, '');
  ...
  base = base.replace(/[\s_-]*(v\d+(\.\d+)?|draft|final|revised|wip|temp|original|early_draft|revised_final)/gi, '');
...
/**
 * Categorizes a file into a high-level media type.
 * Prevents contrasting file types (e.g. PDF documents vs PNG images) from ever being paired as drafts/versions.
 */
export function getFileCategory(filename: string, mimeType?: string): string {
  ...
  if (
    ...
    mime.includes('word') ||
    ['.pdf', '.doc', '.docx', '.rtf', '.odt', '.txt', '.md', '.markdown', '.text'].includes(ext)
  ) {
    return 'document';
  }
  ...
  if (
    ...
    ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.svg', '.tiff'].includes(ext)
  ) {
    return 'image';
  }
...
/**
 * Checks if two files share compatible media categories.
 */
export function areFileTypesCompatible(fileA: DriveFileItem, fileB: DriveFileItem): boolean {
  const catA = getFileCategory(fileA.name, fileA.mimeType);
  const catB = getFileCategory(fileB.name, fileB.mimeType);
  return catA === catB;
}

/**
 * Calculates token-overlap ratio between base title words.
 */
export function calculateWordTitleSimilarity(nameA: string, nameB: string): number {
  ...
  const wordsA = new Set(baseA.split(' ').filter((w) => w.length >= 2));
  const wordsB = new Set(baseB.split(' ').filter((w) => w.length >= 2));
...
export interface SeniorityDecision {
  ...
  signalUsed: 'canonical_export' | 'content_hash' | 'content_statement' | 'modified_timestamp' | 'size_and_name' | 'sha256_checksum' | 'md5_checksum' | 'none';
  reason: string;
  isUncertain: boolean;
...
export function decideNewerVersion(
  ...
  if (matchType === 'exact') {
    ...
    const isGoogleDoc = (f: DriveFileItem) => f.mimeType.startsWith('application/vnd.google-apps');
    const isGDoc = isGoogleDoc(fileA) || isGoogleDoc(fileB);
    const isBothExtracted = fileA.contentStatus === 'extracted' && fileB.contentStatus === 'extracted';

    let exactSignal: 'canonical_export' | 'content_hash' | 'sha256_checksum' | 'md5_checksum' | 'modified_timestamp' = 'modified_timestamp';
    let exactReason = '';
    if (isGDoc) {
      exactSignal = 'canonical_export';
      exactReason = `Content-exact duplicate Google Workspace document verified by canonical export text. ${nameReason}`;
    } else if (isBothExtracted) {
      exactSignal = 'content_hash';
...
export function analyzeDuplicates(
  ...
  const isValidHash = (hash?: string, size?: number): boolean => {
    if (!hash) return false;
    const clean = hash.trim().toLowerCase();
    if (clean.length < 16) return false;
    if (size !== undefined && size > 0) return true;
    return clean !== EMPTY_SHA256 && clean !== EMPTY_MD5;
  };

  // 1. First Pass: Group by exact hash (checking md5Checksum and contentHash)
  // Only index non-empty files with genuinely valid hashes
  // Files with Google Drive md5Checksum get grouped by md5:<hash>; files without it get grouped by sha:<hash>
  const hashGroups = new Map<string, DriveFileItem[]>();
  ...
  for (const file of sortedFiles) {
    ...
    const fileSize = file.size !== undefined ? Number(file.size) : undefined;
    if (isValidHash(file.md5Checksum, fileSize)) {
      primaryKey = `md5:${file.md5Checksum!.trim().toLowerCase()}`;
    } else if (isValidHash(file.contentHash, fileSize)) {
      primaryKey = `sha:${file.contentHash!.trim().toLowerCase()}`;
    } else if (file.contentStatus === 'extracted' && file.content && file.content.trim().length > 0) {
      primaryKey = `text:${normalizeText(file.content)}`;
    }

    if (primaryKey) {
      if (!hashGroups.has(primaryKey)) hashGroups.set(primaryKey, []);
      hashGroups.get(primaryKey)!.push(file);
    }
  }

  // Process candidate exact hash groups
  const processExactGroup = (group: DriveFileItem[]) => {
    // Filter out already resolved files
    const candidates = group.filter((f) => !exactResolvedIds.has(f.id));
    if (candidates.length <= 1) return;

    // Partition candidates into verified identical subgroups
    const verifiedSubgroups: DriveFileItem[][] = [];

    for (const file of candidates) {
      let placed = false;
      for (const subgroup of verifiedSubgroups) {
        const rep = subgroup[0];

        // 1. Must share compatible media category
        if (!areFileTypesCompatible(file, rep)) continue;

        // 2. File size match (if both known and > 0)
        const sizeFile = file.size !== undefined ? Number(file.size) : undefined;
        const sizeRep = rep.size !== undefined ? Number(rep.size) : undefined;
        if (sizeFile !== undefined && sizeRep !== undefined && sizeFile > 0 && sizeRep > 0 && sizeFile !== sizeRep) {
          continue;
        }

        // 3. MD5 checksum verification (if both present)
        if (isValidHash(file.md5Checksum) && isValidHash(rep.md5Checksum)) {
          if (file.md5Checksum!.trim().toLowerCase() !== rep.md5Checksum!.trim().toLowerCase()) {
            continue;
          }
        }

        // 4. Content SHA verification (if both present)
        if (isValidHash(file.contentHash) && isValidHash(rep.contentHash)) {
          if (file.contentHash!.trim().toLowerCase() !== rep.contentHash!.trim().toLowerCase()) {
            continue;
          }
        }

        // 5. Extracted text verification (if both extracted)
        if (
          file.contentStatus === 'extracted' &&
          rep.contentStatus === 'extracted' &&
          file.content &&
          rep.content
        ) {
          const normA = normalizeText(file.content);
          const normB = normalizeText(rep.content);
          if (normA !== normB && file.content.trim() !== rep.content.trim()) {
            continue;
          }
        }
      ...
      if (!placed) {
        verifiedSubgroups.push([file]);
      }
    }

    // Now resolve each verified identical subgroup
    for (const subgroup of verifiedSubgroups) {
      if (subgroup.length <= 1) continue;

      // Select the single best original keeper among the group
      let keeper = subgroup[0];
      for (let k = 1; k < subgroup.length; k++) {
        const cand = subgroup[k];
        const decision = decideNewerVersion(keeper, cand, 'exact', options?.preferences);
        keeper = decision.keeper;
      }

      // Mark keeper as resolved
      exactResolvedIds.add(keeper.id);

      // Pair all other files in subgroup as duplicates of keeper
      for (const other of subgroup) {
        if (other.id === keeper.id) continue;
        exactResolvedIds.add(other.id);

        const isGoogleDoc = (file: DriveFileItem) =>
          file.mimeType.startsWith('application/vnd.google-apps');
        const isGDoc = isGoogleDoc(keeper) || isGoogleDoc(other);

        const isBothExtracted = keeper.contentStatus === 'extracted' && other.contentStatus === 'extracted';
        const comparisonMethod = (isBothExtracted || isGDoc) ? 'text_similarity' : 'binary_checksum_match';

        // Check which hash was used
        let hashVerification = '';
        const keeperSizeNum = keeper.size !== undefined ? Number(keeper.size) : undefined;
        if (isValidHash(keeper.contentHash, keeperSizeNum) && keeper.contentHash === other.contentHash) {
          hashVerification = 'SHA-256 verified';
        } else if (isValidHash(keeper.md5Checksum, keeperSizeNum) && keeper.md5Checksum === other.md5Checksum) {
          hashVerification = 'MD5 checksum verified';
        }

        const isKeeperCopy = hasCopySuffix(keeper.name);
        const isOtherCopy = hasCopySuffix(other.name);
        const nameReason = isKeeperCopy !== isOtherCopy
          ? `"${keeper.name}" is retained as original, while duplicate copy "${other.name}" has a copy suffix.`
          : `"${keeper.name}" modified ${new Date(keeper.modifiedTime).toLocaleDateString()} vs "${other.name}" on ${new Date(other.modifiedTime).toLocaleDateString()}.`;
  ...
  for (const [, group] of hashGroups) {
    processExactGroup(group);
  }

  // 1.5. Near-Identical Filename + Exact File Size Match (when content is unavailable)
  // Per specification: Filename and size alone CANNOT become cleanup-eligible.
  // Must be flagged as probable-candidate requiring manual review.
  for (let i = 0; i < sortedFiles.length; i++) {
    ...
    for (let j = i + 1; j < sortedFiles.length; j++) {
      ...
      if (!areFileTypesCompatible(fileA, fileB)) continue;

      // If both have MD5 checksums and they differ, they cannot be exact duplicates
      if (isValidHash(fileA.md5Checksum) && isValidHash(fileB.md5Checksum)) {
        if (fileA.md5Checksum!.trim().toLowerCase() !== fileB.md5Checksum!.trim().toLowerCase()) {
          continue;
        }
      }
      ...
      const isNearName =
        areFilenamesNearIdentical(fileA.name, fileB.name) ||
        (getBaseTitle(fileA.name).toLowerCase().length >= 4 &&
          getBaseTitle(fileB.name).toLowerCase().length >= 4 &&
          (getBaseTitle(fileA.name).toLowerCase().startsWith(getBaseTitle(fileB.name).toLowerCase()) ||
            getBaseTitle(fileB.name).toLowerCase().startsWith(getBaseTitle(fileA.name).toLowerCase())));
      ...
      if (isNearName && hasMatchingSize) {
        ...
        const candidateMatch: DuplicateMatch = {
          ...
          confidence: 0.60,
          reason: `Probable candidate: Matched by identical file size (${formatBytes(fileA.size)}) and near-identical filename alone — content could not be read. Cannot become cleanup-eligible without verified content. ${nameReason}`,
          signalUsed: 'size_and_name',
  ...
  if (!options?.exactOnly) {
    ...
    for (let i = 0; i < remainingFiles.length; i++) {
      ...
      for (let j = i + 1; j < remainingFiles.length; j++) {
        ...
        // RULE: Files of different media categories (e.g. PDF documents vs PNG images) can NEVER be version pairs
        if (!areFileTypesCompatible(fileA, fileB)) {
          continue;
        }

        const titleSim = calculateTitleSimilarity(fileA.name, fileB.name);
        const wordSim = calculateWordTitleSimilarity(fileA.name, fileB.name);
        const baseA = getBaseTitle(fileA.name);
        const baseB = getBaseTitle(fileB.name);

        const hasStrongTitleRelationship =
          areFilenamesNearIdentical(fileA.name, fileB.name) ||
          (baseA.length >= 3 && baseB.length >= 3 && (baseA === baseB || baseA.includes(baseB) || baseB.includes(baseA))) ||
          titleSim >= 0.50 ||
          wordSim >= 0.60;
        ...
        if (!hasRealContentA || !hasRealContentB) {
          // ONLY flag as "Uncertain — Content Not Readable" if there is a genuine title relationship
          if (hasStrongTitleRelationship) {
        ...
        const isConfirmedSameSubject =
          (hasStrongTitleRelationship && meetsContentGate) ||
          contentSim >= 0.70;
  ...
  const enrichWithDivergence = (m: DuplicateMatch): DuplicateMatch => {
    ...
    } else if (m.type === 'content-exact' || m.signalUsed === 'canonical_export' || m.signalUsed === 'content_hash') {
      summaryMessage = 'Content-exact duplicate document verified by canonical export text. 100% safe to clean.';
    } else if (m.type === 'byte-exact') {
      summaryMessage = 'Exact byte-identical binary file. Verified by identical checksum.';
    } else if (m.type === 'exact') {
      summaryMessage = 'Exact duplicate. 100% safe to clean.';
