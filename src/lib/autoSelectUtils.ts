...
import { isMatchSelectable } from './cleanupActionGate';
...
export function evaluateAutoSelectMatchIds(
  ...
  for (const match of matches) {
    // Strict safety check: probable candidates, unreadable files, or non-deletion eligible items can NEVER be selected
    if (!isMatchSelectable(match)) {
      continue;
    }
